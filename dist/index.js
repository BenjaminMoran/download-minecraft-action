...
/* harmony import */ var _actions_io__WEBPACK_IMPORTED_MODULE_4__ = __nccwpck_require__(436);
...
try {
  ...
  if (error instanceof Error) _actions_core__WEBPACK_IMPORTED_MODULE_2__.setFailed(error.message)
}

function getInputs() {
  const version = _actions_core__WEBPACK_IMPORTED_MODULE_2__.getInput('version', { required: true })
  const files = _actions_core__WEBPACK_IMPORTED_MODULE_2__.getInput('files', { required: true }).split(' ')
  const destination = _actions_core__WEBPACK_IMPORTED_MODULE_2__.getInput('destination') || '.'
  return { version, files, destination }
}

function setOutputs(outputs) {
  _actions_core__WEBPACK_IMPORTED_MODULE_2__.setOutput('output-paths', outputs.outputPaths.join('\n'))
}

async function main(inputs) {
  const { version, files, destination } = inputs

  const httpClient = new _actions_http_client__WEBPACK_IMPORTED_MODULE_3__.HttpClient(USER_AGENT)

  _actions_core__WEBPACK_IMPORTED_MODULE_2__.debug('Downloading version manifest')
  ...
  _actions_core__WEBPACK_IMPORTED_MODULE_2__.debug(`Downloading version data for ${version}`)
  ...
  _actions_core__WEBPACK_IMPORTED_MODULE_2__.info(`Found Minecraft version ${version}`)
  ...
  _actions_core__WEBPACK_IMPORTED_MODULE_2__.info(`Downloaded ${files.length} Minecraft files`)
...
async function downloadFile(httpClient, key, url, outputDir) {
  ...
  const outputPath = path__WEBPACK_IMPORTED_MODULE_1__.join(outputDir, key + '.' + extension)
  _actions_core__WEBPACK_IMPORTED_MODULE_2__.debug(`Downloading ${url} to ${outputPath}`)

  _actions_io__WEBPACK_IMPORTED_MODULE_4__.mkdirP(outputDir)
  const response = await httpClient.get(url)
  if (response.message.statusCode !== 200) {
    throw new Error(`Failed to download ${key} from ${url}`)
  }
  response.message.pipe(fs__WEBPACK_IMPORTED_MODULE_0__.createWriteStream(outputPath))

  _actions_core__WEBPACK_IMPORTED_MODULE_2__.debug(`Downloaded ${url} to ${outputPath}`)
  return outputPath
}
...
/******/ var __webpack_exports__ = __nccwpck_require__(5);
